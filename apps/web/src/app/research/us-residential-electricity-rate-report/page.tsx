...
import { createReportStructuredData, serializeStructuredData } from '@/lib/structured-data';
...
export default async function NationalRateReportPage() {
  ...
  const reportJsonLd = createReportStructuredData({
    title: 'U.S. Residential Electricity-Rate Report',
    description:
      'Comprehensive national data report analyzing U.S. residential electricity rates, state rankings, month-over-month price changes, annual trends, and household energy charge benchmarks.',
    path: '/research/us-residential-electricity-rate-report',
    datePublished: '2026-07-24',
    dateModified: report.generatedAt ? report.generatedAt.slice(0, 10) : '2026-07-24',
    reportingPeriod: report.reportingPeriod ? report.reportingPeriod.slice(0, 7) : '2026-04',
  });
  ...
  return (
    <PageContainer>
      <main data-report-period={report.reportingPeriod || '2026-04'}>
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: serializeStructuredData(reportJsonLd) }}
        />
          ...
          <>
            ...
            <section className={styles.summaryGrid} aria-label="Report Executive Summary">
              <div className={styles.summaryCard}>
                <span className={styles.summaryLabel}>Validated Reporting Period</span>
                <strong className={styles.summaryValue}>
                  {report.reportingPeriodFormatted || report.reportingPeriod}
                </strong>
                <small className={styles.summaryNote}>
                  Latest complete reporting period available in Energy Bill Lab’s validated
                  database. The U.S. EIA may have published a newer monthly release that has not yet
                  completed our import and validation process.
                </small>
              </div>
            ...
            <section className={styles.sectionCard} aria-labelledby="monthly-changes-heading">
              ...
              <p className={styles.sectionSubtitle}>
                States experiencing the largest single-month residential rate increases and
                decreases compared to the preceding calendar month.
              </p>
              <p className={styles.freshnessNotice}>
                <strong>Interpretation Guidance:</strong> Monthly movements should be interpreted
                cautiously. EIA state prices represent average residential revenue per
                kilowatt-hour, not a single utility tariff. Preliminary reporting, billing-cycle
                timing, supplier settlements, and customer mix can create unusually large
                        ...
                        {report.largestMonthlyIncreases.map((item) => (
                          <tr key={item.code}>
                            ...
                            <td className={styles.numericCol}>
                              {item.currentRateCents.toFixed(2)}¢
                            </td>
                            <td className={styles.increaseCol}>+{item.changeCents.toFixed(2)}¢</td>
                            <td className={styles.increaseCol}>
                              +{item.changePercent.toFixed(1)}%
                              {item.isLargeMonthlyMovement && (
                                <span title="Unusually large monthly change (>=20%)"> *</span>
                              )}
                            </td>
                          </tr>
                        ))}
                      ...
                      <tbody>
                        {report.largestMonthlyDecreases.map((item) => (
                          <tr key={item.code}>
                            ...
                            <td className={styles.decreaseCol}>
                              {item.changePercent.toFixed(1)}%
                              {item.isLargeMonthlyMovement && (
                                <span title="Unusually large monthly change (>=20%)"> *</span>
                              )}
            ...
            <section className={styles.sectionCard} aria-labelledby="annual-changes-heading">
              ...
              <p className={styles.sectionSubtitle}>
                States with the highest year-over-year rate shifts compared to the exact same
                  ...
                  <tbody>
                    {report.householdExamples.map((ex) => (
                      <tr key={ex.kwh}>
                        ...
                        <td className={styles.numericCol}>
                          {ex.nationalCostUsd !== null
                            ? `$${ex.nationalCostUsd.toFixed(2)}`
                            : 'N/A'}
                        </td>
                        <td>
            ...
            <section className={styles.sectionCard} aria-labelledby="methodology-heading">
              ...
              <div className={styles.prose}>
                ...
                <p>
                  All rate figures are derived from Form EIA-861M Monthly Retail Sales of
                  Electricity reports published by the U.S. Energy Information Administration (EIA).
                  The average residential rate is calculated by dividing total residential revenue
                  (in U.S. dollars) by total residential energy sales (in kilowatt-hours) for each
                  state.
                </p>
            ...
            <section className={styles.publisherCard} aria-labelledby="publisher-heading">
              ...
              <p className={styles.publisherText}>
                This report is researched, built, and technically published by Jaynesh Shingala for
                Energy Bill Lab.
              </p>
