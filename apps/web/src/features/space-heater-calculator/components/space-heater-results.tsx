...
import styles from './space-heater-results.module.css';

type SpaceHeaterResultsProps = {
  result: SpaceHeaterCostResult;
  periodDays: number;
};

export function SpaceHeaterResults({ result, periodDays }: SpaceHeaterResultsProps) {
    ...
    <div className={styles.resultsContainer}>
      ...
      <div className={styles.primaryResultCard}>
        <div className={styles.primaryTitle}>
          Estimated Space Heating Operating Cost ({periodDays} Days)
        </div>
        <div className={styles.primaryValue}>${result.periodCostUsd.toFixed(2)} USD</div>
        <div className={styles.primarySubtext}>
          Total heating energy: <strong>{result.periodKwh.toFixed(2)} kWh</strong> for{' '}
          {result.quantity} heater
          {result.quantity > 1 ? 's' : ''} ({result.totalRatedWatts}W total) @{' '}
          {result.rateCentsPerKwhUsed.toFixed(2)}¢/kWh ({result.dutyCyclePercentUsed}% duty cycle)
        </div>
      </div>
      ...
      <div className={styles.grid}>
        ...
        {/* Electrical Load & Active Power Draw */}
        <div className={styles.metricCard}>
          <div className={styles.metricLabel}>Connected Electrical Load</div>
          <div className={styles.metricVal}>{result.totalRatedWatts} W Total</div>
      ...
      <div className={styles.scenarioSection}>
        <div className={styles.scenarioTitle}>Heating Cost Comparison Scenarios</div>
        <Table
        ...
        <div style={{ fontSize: 12, color: '#8c8c8c', marginTop: 8 }}>
          * Estimated difference based on the entered assumptions. Actual bill impact depends on
          thermostat cycling, room conditions, utility pricing, other household usage, and fixed
          charges.
        </div>
      </div>
    </div>
