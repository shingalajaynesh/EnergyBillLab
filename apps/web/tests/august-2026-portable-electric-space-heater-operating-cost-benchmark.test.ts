...
import { august2026PortableElectricSpaceHeaterOperatingCostBenchmark } from '../src/content/insights/articles/august-2026-portable-electric-space-heater-operating-cost-benchmark';
import {
...
describe('August 2026 Portable Electric Space Heater Operating Cost Benchmark Insight', () => {
  ...
  it('2. is registered in the global insights registry', () => {
    ...
    expect(found?.id).toBe(
      'insight-2026-08-06-august-portable-electric-space-heater-operating-cost-benchmark',
    );
  });
  ...
  it('4. activates home-energy-costs category with at least 3 published articles', () => {
    const homeEnergyInsights = getInsightsByCategory(
      'home-energy-costs',
      '2026-08-05T12:00:00.000Z',
    );
    expect(homeEnergyInsights.length).toBeGreaterThanOrEqual(1);
  });

  it('5. adheres to strict author and privacy boundaries', () => {
    const record = august2026PortableElectricSpaceHeaterOperatingCostBenchmark;
