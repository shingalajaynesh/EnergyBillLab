...
import { august2026ElectricDishwasherKwhOperatingCostBenchmark } from '../src/content/insights/articles/august-2026-electric-dishwasher-kwh-operating-cost-benchmark';
import {
...
describe('August 2026 Electric Dishwasher kWh Consumption & Operating Cost Benchmark Insight', () => {
  it('1. passes full automated registry record validation', () => {
    const result = validateInsightRecord(august2026ElectricDishwasherKwhOperatingCostBenchmark);
  ...
  it('3. is included in published insights', () => {
    const published = getPublishedInsights('2026-08-11T12:00:00.000Z');
    const found = published.find(
      (item) => item.slug === 'august-2026-electric-dishwasher-kwh-operating-cost-benchmark',
    );
    expect(found).toBeDefined();
  ...
  it('4. activates appliances category with published articles', () => {
    const applianceInsights = getInsightsByCategory('appliances', '2026-08-11T12:00:00.000Z');
    expect(applianceInsights.length).toBeGreaterThanOrEqual(1);
  });

  it('5. adheres to strict author and privacy boundaries', () => {
    const record = august2026ElectricDishwasherKwhOperatingCostBenchmark;
