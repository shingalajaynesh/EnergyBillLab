...
import { august2026ElectricDehumidifierKwhOperatingCostBenchmark } from '../src/content/insights/articles/august-2026-electric-dehumidifier-kwh-operating-cost-benchmark';
import {
...
describe('August 2026 Electric Dehumidifier kWh Consumption & Operating Cost Benchmark Insight', () => {
  it('1. passes full automated registry record validation', () => {
    const result = validateInsightRecord(august2026ElectricDehumidifierKwhOperatingCostBenchmark);
  ...
  it('2. is registered in the global insights registry', () => {
    ...
    expect(found?.id).toBe('august-2026-electric-dehumidifier-kwh-operating-cost-benchmark');
  });

  it('3. is included in published insights', () => {
    const published = getPublishedInsights('2026-08-13T12:00:00.000Z');
    const found = published.find(
      (item) => item.slug === 'august-2026-electric-dehumidifier-kwh-operating-cost-benchmark',
    );
    expect(found).toBeDefined();
  ...
  it('4. activates appliances category with published articles', () => {
    const applianceInsights = getInsightsByCategory('appliances', '2026-08-13T12:00:00.000Z');
    expect(applianceInsights.length).toBeGreaterThanOrEqual(1);
  });

  it('5. adheres to strict author and privacy boundaries', () => {
    const record = august2026ElectricDehumidifierKwhOperatingCostBenchmark;
