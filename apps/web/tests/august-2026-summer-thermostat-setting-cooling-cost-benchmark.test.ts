...
import { august2026SummerThermostatSettingCoolingCostBenchmark } from '../src/content/insights/articles/august-2026-summer-thermostat-setting-cooling-cost-benchmark';
import {
...
describe('August 2026 Summer Thermostat Setting Cooling Cost Benchmark Insight', () => {
  it('1. passes full automated registry record validation', () => {
    const result = validateInsightRecord(august2026SummerThermostatSettingCoolingCostBenchmark);
    expect(result.valid).toBe(true);
    expect(result.errors).toEqual([]);
  });

  it('2. is registered in the global insights registry', () => {
    const found = getInsightBySlug('august-2026-summer-thermostat-setting-cooling-cost-benchmark');
    expect(found).toBeDefined();
    expect(found?.id).toBe('august-2026-summer-thermostat-setting-cooling-cost-benchmark');
  });

  it('3. is included in published insights', () => {
    const published = getPublishedInsights('2026-08-18T00:00:00.000Z');
    const found = published.find(
      (item) => item.slug === 'august-2026-summer-thermostat-setting-cooling-cost-benchmark',
    );
    expect(found).toBeDefined();
  ...
  it('4. activates home-energy-costs category with published articles', () => {
    const homeCostInsights = getInsightsByCategory('home-energy-costs', '2026-08-18T00:00:00.000Z');
    expect(homeCostInsights.length).toBeGreaterThanOrEqual(5);
  });

  it('5. adheres to strict author and privacy boundaries', () => {
    const record = august2026SummerThermostatSettingCoolingCostBenchmark;
