...
import { august2026InductionVsElectricVsGasCooktopEnergyCostBenchmark } from '../src/content/insights/articles/august-2026-induction-vs-electric-vs-gas-cooktop-energy-cost-benchmark';
import {
...
describe('August 2026 Induction vs Electric vs Gas Cooktop Energy Cost Benchmark Insight', () => {
  ...
  it('3. is included in published insights', () => {
    ...
    const found = published.find(
      (item) =>
        item.slug === 'august-2026-induction-vs-electric-vs-gas-cooktop-energy-cost-benchmark',
    );
    expect(found).toBeDefined();
  ...
  it('4. activates appliances category with published articles', () => {
    ...
    expect(applianceInsights.length).toBeGreaterThanOrEqual(4);
  });

  it('5. adheres to strict author and privacy boundaries', () => {
    const record = august2026InductionVsElectricVsGasCooktopEnergyCostBenchmark;
